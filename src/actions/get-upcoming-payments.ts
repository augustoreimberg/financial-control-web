...
export const getUpcomingPayments = async (clientToken?: string): Promise<UpcomingPaymentsResponse> => {
  try {
    const token = await getAuthToken(clientToken)

    const url = `${process.env.API_URL}/payment/upcoming`
    console.log(`Buscando próximos vencimentos: ${url}`)
    ...
    if (!response.ok) {
      ...
      throw new Error(`Falha ao buscar próximos vencimentos: ${response.status} - ${errorText || "Sem detalhes"}`)
    }
    ...
    if (data.payments && Array.isArray(data.payments)) {
      ...
      console.log("Próximos vencimentos transformados:", transformedPayments.length)
      return {
    ...
    return {
      data: null,
      error: "Erro desconhecido ao buscar próximos vencimentos",
    }
  }
}

...
export default function PoliciesPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const token =
          Cookies.get('client_token') || localStorage.getItem('accessToken')
          ...
          paymentsResult.data?.map((payment) => {
            ...
            const randomClient =
              clientsResult.data && clientsResult.data.length > 0
                ? clientsResult.data[
                    Math.floor(Math.random() * clientsResult.data.length)
                  ]
                : null
  ...
  const handleFilter = async (filters: PaymentsFiltersType) => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : 'Erro ao filtrar pagamentos'
      )
    } finally {
      setIsLoading(false)
    }

...
export function PolicyCalendar({
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "PAID":
                return (
                    <Badge className="bg-green-600/20 text-green-500 border-green-800/30">
                        Pago
                    </Badge>
                );
            case "PENDING":
                return (
                    <Badge className="bg-yellow-600/20 text-yellow-500 border-yellow-800/30">
                        Pendente
                    </Badge>
                );
            case "DEFEATED":
                return (
                    <Badge className="bg-red-600/20 text-red-500 border-red-800/30">
                        Vencido
                    </Badge>
                );
            default:
                return (
                    <Badge className="bg-zinc-600/20 text-zinc-400 border-zinc-700/30">
                        {status}
                    </Badge>
                );
        }
                            ...
                            <CardContent className="p-4 h-full flex flex-col">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center">
                                        ...
                                        {hasMultiplePayments && (
                                            <Badge className="ml-2 bg-red-900/30 text-red-400 border-red-800/50">
                                                {payments.length}
                                            </Badge>
                                        )}
                                    </div>
